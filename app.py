...
# ---------------------------------------------------------
# TAB 1: REGRESYON — 1 HAFTALIK (7 GÜN)
# ---------------------------------------------------------
with tab1:
    st.header("Senaryo Bazlı Büyüklük Tahmini (1 Haftalık)")
    st.info("Aşağıdaki parametreleri girerek seçtiğiniz tarihten itibaren 7 günlük tahmini büyüklük (Mw) üretin.")
    ...
    if rf_reg is not None:
        ...
        if st.button("1 Haftalık Tahmin Üret", type="primary"):
            try:
                ...
                st.line_chart(results.set_index("Tarih")[["Tahmini Mw"]])
...
# ---------------------------------------------------------
# TAB 2: SINIFLANDIRMA (RİSK ANALİZİ) — Progress Bar kaldırıldı
# ---------------------------------------------------------
with tab2:
